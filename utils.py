...
def create_test_file_list(target_dir,
                          ...
                          prefix_anomaly='anomaly',
                          ext='wav'):
    normal_files_path = f'{target_dir}/{dir_name}/*{prefix_normal}*.{ext}'
    normal_files = sorted(glob.glob(normal_files_path))
    normal_labels = np.zeros(len(normal_files))

    anomaly_files_path = f'{target_dir}/{dir_name}/*{prefix_anomaly}*.{ext}'
    anomaly_files = sorted(glob.glob(anomaly_files_path))
...
def path_to_dict(process_machines,
    ...
    for index, target_dir in enumerate(sorted(dirs)):
        ...
        dir = os.path.join(target_dir, dir_name)
        for root, directories, files in os.walk(dir):
            for filename in files:
                filepath = os.path.join(root, filename)
                sec_num = filename.split('_')[1]
                path_list[machine_type + '_sec_' + sec_num].append(filepath)
                # file_paths.append(filepath)

        # path_list[machine_type] = file_paths
        # print(f'{data_type} {machine_type}  were split to {len(file_paths)} wav files!')

    # path_list = list(chain.from_iterable(path_list))
    if not os.path.exists(os.path.split(root_folder)[0]):
